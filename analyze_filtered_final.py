...
def main():
    print("Training FINAL Model (Filtered 20+ Apps Only)...")
    
    if not os.path.exists("Pl_transfers_20apps.csv"):
        print("Error: Dataset not found.")
        return
        
    df = pd.read_csv("Pl_transfers_20apps.csv")
    
    # 1. Split Data (Cutoff 2022)
    split_year = 2022
    train_df = df[df['Season_Control'] < split_year].copy()
    test_df_full = df[df['Season_Control'] >= split_year].copy()
    
    print(f"Training Set (Historic 20+ Apps): {len(train_df)} players")
    print(f"Test Set (Modern 20+ Apps): {len(test_df_full)} players")
    ...
    # Tuned for smaller dataset (less depth to prevent overfitting)
    ...
    def evaluate_cohort(name, min_ga):
        # Filter Test Set for this cohort
